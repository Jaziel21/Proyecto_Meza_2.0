...
def eliminar_venta(request, id):
    ...
    if request.method == 'POST':
        try:
            # SIEMPRE restaurar stock, sin importar el estado
            for detalle in venta.detalles.all():
                libro = detalle.libroid
                libro.stock += detalle.cantidad
                libro.save()
                print(f"✅ Stock restaurado: {libro.titulo} (+{detalle.cantidad})")
            
            # Guardar ID para el mensaje antes de eliminar
            venta_id = venta.ventaid
            
            # Eliminar la venta
            venta.delete()
            
            messages.success(request, f'✅ Venta #{venta_id} eliminada correctamente. Stock restaurado.')
...
def cancelar_venta(request, venta_id):
    """Cancelar una venta (cambia estado y restaura stock)"""
    venta = get_object_or_404(Venta, ventaid=venta_id)
    
    if request.method == 'POST':
        try:
            # 1. Restaurar stock de cada libro
            for detalle in venta.detalles.all():
                libro = detalle.libroid
                libro.stock += detalle.cantidad
                libro.save()
            ...
            messages.success(request, f'✅ Venta #{venta.ventaid} cancelada. Stock restaurado.')
            return redirect('admin_ventas')
    ...
    # Si es GET, redirigir con error
    messages.error(request, 'Método no permitido. Use el formulario de cancelación.')
    return redirect('admin_ventas')
# =============================================
